...
# coding: utf-8
"""
Programa: eficiencia_energetica_IoT_128_clusters_V39.py
...
save_path = r'C:\Users\mauri\Documents\Mestrado_OFICIAL\DEFESA MESTRADO\eficiencia_128/V39'
...
def main():
 ...
 # Gráfico 10: Eficiência Energética de Clusters na RSSF (ajustado para 128 clusters)
    inverse_energy_consumption = []
    for num_clusters, cluster_data in all_cluster_data.items():
        if num_clusters > 128:  # Limitar ao máximo de 128 clusters
            continue
        total_energy = sum(item['total_transmission_power'] + item['ch_to_sink_power'] for item in cluster_data['data'])
        inverse_consumption = 1 / total_energy if total_energy != 0 else float('inf')
        inverse_energy_consumption.append((num_clusters, inverse_consumption))
    ...
    ax.set_title("Eficiência Energética de Clusters na RSSF (até 128 clusters)")
    ax.set_xlabel("Número de Clusters")
    ax.set_ylabel("1/Energia consumida pelos clusters (W)")
    plt.xticks(df_inverse_consumption['Número de Clusters'], rotation=45)
    plt.grid(axis='y', linestyle='--', linewidth=0.5)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, "Eficiencia_Energetica_Por_Quantidade_Clusters_128.png"), bbox_inches='tight')
    plt.show()
    display(Markdown(r"$\text{Eficiência Energética de Clusters na RSSF} = \frac{1}{\text{Energia consumida pelos clusters}}$"))
    display(Markdown("**Este gráfico apresenta a Eficiência Energética de quantidades de clusters na RSSF (limitado a 128 clusters).**"))
