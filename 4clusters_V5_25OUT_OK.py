...
def main():
    ...
    plt.title(f"Trajetória dos Centroides e Cluster Heads (Iterações: {var_iteration})")
    plt.xlabel("Eixo X")
    plt.ylabel("Eixo Y")
    plt.legend()
    plt.savefig(os.path.join(save_path, f"Grafico1_Trajetoria_Centroides_{var_iteration}_iteracoes.png"))
    plt.show()
    ...
    ax.set_xlim(-10, 110)
    ax.set_ylim(-10, 110)
    ax.set_title(f"Clusters com Alcance Máximo e Cluster Heads (Iterações: {var_iteration})")
    ax.set_xlabel("Eixo X")
    ax.set_ylabel("Eixo Y")
    plt.legend()
    plt.savefig(os.path.join(save_path, f"Grafico2_Distribuicao_Clusters_{var_iteration}_iteracoes.png"))
    plt.show()
    ...
    plt.title(f"Potência de Transmissão dos Cluster Heads para Base Station (Iterações: {var_iteration})")
    plt.xticks(range(1, 5), [f'CH{i+1}' for i in range(4)])
    plt.savefig(os.path.join(save_path, f"Grafico3_Potencia_Cluster_Heads_{var_iteration}_iteracoes.png"))
    plt.show()
    ...
    plt.title(f"Potência Total Gasta por Cluster (Ordem Crescente) (Iterações: {var_iteration})")
    plt.xticks(df_clusters_sorted.index + 1, [f'Cluster {i}' for i in df_clusters_sorted["Cluster"]])
    plt.savefig(os.path.join(save_path, f"Grafico4_Potencia_Total_Clusters_{var_iteration}_iteracoes.png"))
    plt.show()
