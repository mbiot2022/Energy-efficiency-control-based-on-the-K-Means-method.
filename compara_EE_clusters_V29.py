...
# coding: utf-8
"""
Programa: compara_EE_clusters_V29.py
...
save_path = r'C:\Users\mauri\Documents\Mestrado_OFICIAL\DEFESA MESTRADO\comparaclusters\V22'
...
def main():
    ...
    # Gráficos de Potências por Cluster com valores espaçados no eixo X para legibilidade
    for num_clusters in cluster_sizes:
        ...
        plt.xticks(rotation=45, ha='right', fontsize=8)  # Ajuste para melhorar a legibilidade
    ...
    # Gráfico 4: Correlação Potência CH-Sink / Potência Total (%)
    ...
    # Gráfico 5:
    # Gráfico 5: Consumo energético de Sensores por Clusters na RSSF
    results = []
    for num_clusters, cluster_data in all_cluster_data.items():
        ...
        eficiencia_energetica = pot_sensores_clusters_i_n / pot_tot_rssf
    ...
    plt.show()
    display(Markdown(r"$\text{EE\_Sensores} = \frac{\text{Pot\_Total\_Sensores}}{\text{Pot\_Total\_RSSF}}$"))
    display(Markdown("**Este gráfico representa o consumo energético dos sensores em relação à potência total da RSSF.**"))

    # Gráfico 6: Potência Total CH-Sink
    fig, ax = plt.subplots()
    ...
    # Gráfico 7: Eficiência energética de Clusters na RSSF
    comparative_results = []
    for num_clusters, cluster_data in all_cluster_data.items():
        data = cluster_data['data']
    ...
    # Gráfico 8: Eficiência energética média dos Sensores -> Clusters Heads na RSSF
    fig, ax = plt.subplots()
    ...
    # Gráfico 9: Eficiência energética dos Sensores -> Clusters Heads na RSSF
    fig, ax = plt.subplots()
    ...
    fig, ax = plt.subplots(figsize=(12, 7))
    ax.plot(df_inverse_consumption['Número de Clusters'], df_inverse_consumption['1/Energia consumida pelos clusters (W)'], marker='o', linestyle='-', color='orange')
    ax.set_title("Eficiência Energética de Clusters na RSSF")
    ax.set_xlabel("Número de Clusters")
    ax.set_ylabel("1/Energia consumida pelos clusters (W)")
    plt.xticks(df_inverse_consumption['Número de Clusters'], rotation=45)
    plt.grid(axis='y', linestyle='--', linewidth=0.5)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, "Eficiencia_Energetica_Por_Quantidade_Clusters.png"), bbox_inches='tight')
    plt.show()
    display(Markdown(r"$\text{Eficiência Energética de Clusters na RSSF} = \frac{1}{\text{Energia consumida pelos clusters}}$"))
    display(Markdown("**Este gráfico apresenta a Eficiência Energética de quantidades de clusters na RSSF.**"))
    display(Markdown(r"$\text{Energia Consumida} = \text{Pot\_Transmissão\_Sensores + Pot\_CH\_Sink}$"))
